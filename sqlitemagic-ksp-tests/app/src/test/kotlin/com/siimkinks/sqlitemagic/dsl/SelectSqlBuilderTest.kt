...
import com.siimkinks.sqlitemagic.model.ImmutableValueWithFields
...
class SelectSqlBuilderTest : DSLTests {
  ...
  fun whereCondition() {
    ...
    val expectedLongWhereClause =
      "((((((((magazine.name!=? AND magazine.name IS NOT NULL) AND magazine.name IS NULL) " +
    ...
    expected =
      "$expectedBase(((magazine.nr_of_releases IN (?,?,?) AND magazine.nr_of_releases NOT IN (?,?,?)) AND magazine.nr_of_releases IN (?)) AND magazine.nr_of_releases NOT IN (?)) "
    ...
    expected =
      "$expectedBase(magazine.nr_of_releases IN (?,?,?) OR (magazine.nr_of_releases BETWEEN ? AND ? AND magazine.name IS NOT NULL)) "
    (SELECT FROM MAGAZINE WHERE (`in` OR (between AND titleIsNotNull)))
      .isEqualTo(expected, "1910", "1999", "1920", "1910", "2000")

    expected =
      "$expectedBase(magazine.nr_of_releases IN (?,?,?) AND ((magazine.name!=? OR magazine.name IS NOT NULL) OR magazine.name IS NULL)) "
  ...
  fun whereConditionAliased() {
    ...
    expected =
      "$expectedBase(m.nr_of_releases IN (?,?,?) OR (m.nr_of_releases BETWEEN ? AND ? AND m.name IS NOT NULL)) "
  ...
  fun betweenComplex() {
    ...
    val authorId = checkNotNull(randomAuthor.id)
    val authorId2 = checkNotNull(randomAuthor2.id)

    var expected = expectedBase + "WHERE magazine.author BETWEEN ? AND ? "
    (SELECT
        FROM MAGAZINE
        WHERE (MAGAZINE.AUTHOR BETWEEN (authorId AND authorId2)))
      .isEqualTo(expected, authorId.toString(), authorId2.toString())

    expected = expectedBase + "WHERE magazine.author BETWEEN ? AND author.id "
    (SELECT
        FROM MAGAZINE
        WHERE (MAGAZINE.AUTHOR BETWEEN (authorId AND AUTHOR.ID)))
      .isEqualTo(expected, authorId.toString())

    expected = expectedBase + "WHERE magazine.author BETWEEN author.id AND ? "
    (SELECT
        FROM MAGAZINE
        WHERE (MAGAZINE.AUTHOR BETWEEN (AUTHOR.ID AND authorId)))
      .isEqualTo(expected, authorId.toString())
    ...
    (SELECT
        FROM MAGAZINE
        WHERE (MAGAZINE.AUTHOR BETWEEN (MAGAZINE.AUTHOR AND authorId)))
      .isEqualTo(expected, authorId.toString())
  }
  ...
  fun betweenComplexAliased() {
    ...
    (SELECT
        FROM m
        WHERE (m.AUTHOR BETWEEN (MAGAZINE.AUTHOR AND authorId)))
      .isEqualTo(expected, authorId.toString())

    expected = expectedBase + "WHERE m.author BETWEEN m.author AND ? "
    (SELECT
        FROM m
        WHERE (m.AUTHOR BETWEEN (m.AUTHOR AND authorId)))
      .isEqualTo(expected, authorId.toString())
  }
  ...
  fun complexExprWithSameType() {
    val randomAuthor = Author.newRandom()
    val randomAuthorId = checkNotNull(randomAuthor.id)

    assertComplexSameTypeExpr("=?", randomAuthor, MAGAZINE.AUTHOR IS randomAuthorId)
    assertComplexSameTypeExpr("!=?", randomAuthor, MAGAZINE.AUTHOR.isNot(randomAuthorId))
    assertComplexSameTypeExpr(" IN (?)", randomAuthor, MAGAZINE.AUTHOR.`in`(randomAuthorId))
    assertComplexSameTypeExpr(" IN (?)", randomAuthor, MAGAZINE.AUTHOR.`in`(randomAuthorId))
    assertComplexSameTypeExpr(" NOT IN (?)", randomAuthor, MAGAZINE.AUTHOR.notIn(randomAuthorId))
    assertComplexSameTypeExpr(">?", randomAuthor, MAGAZINE.AUTHOR.greaterThan(randomAuthorId))
    assertComplexSameTypeExpr(">=?", randomAuthor, MAGAZINE.AUTHOR.greaterOrEqual(randomAuthorId))
    assertComplexSameTypeExpr("<?", randomAuthor, MAGAZINE.AUTHOR.lessThan(randomAuthorId))
    assertComplexSameTypeExpr("<=?", randomAuthor, MAGAZINE.AUTHOR.lessOrEqual(randomAuthorId))
  }
  ...
  fun complexExprWithEquivalentType() {
    ...
    assertComplexEquivalentTypeExpr(" IN (?)", authorId, MAGAZINE.AUTHOR IN arrayOf(authorId))
    assertComplexEquivalentTypeExpr(" NOT IN (?)", authorId, MAGAZINE.AUTHOR NOT_IN arrayOf(authorId))
  ...
  private fun assertComplexSameTypeExpr(operator: String, author: Author, expr: Expr) {
  ...
  fun exprComplexAliased() {
    ...
    (SELECT
        FROM m
        WHERE (m.AUTHOR IS checkNotNull(randomAuthor.id)))
  ...
  fun joinComplex() {
    ...
    (SELECT
        FROM MAGAZINE
        LEFT_JOIN (MAGAZINE ON (MAGAZINE.AUTHOR IS checkNotNull(randomAuthor.id))))
      .isEqualTo(expected, randomAuthor.id.toString())
  }
  ...
  fun joinComplexAliased() {
    ...
    (SELECT
        FROM m
        LEFT_JOIN (MAGAZINE.on(m.AUTHOR IS checkNotNull(randomAuthor.id))))
      .isEqualTo(expected, randomAuthor.id.toString())

    expected = expectedBase + "LEFT JOIN magazine AS m ON m.author=? "
    (SELECT
        FROM m
        LEFT_JOIN (m.on(m.AUTHOR IS checkNotNull(randomAuthor.id))))
      .isEqualTo(expected, randomAuthor.id.toString())
  }
  ...
  fun groupByTest() {
    ...
    (SELECT
      ...
      .isEqualTo(
        expectedBase + "LEFT JOIN author GROUP BY author.boxed_boolean HAVING author.primitive_boolean=? ",
  ...
  fun groupByTestAliased() {
    ...
    (SELECT
      ...
      .isEqualTo(
        expectedBase + "LEFT JOIN author AS a GROUP BY a.boxed_boolean HAVING a.primitive_boolean=? ",
        BooleanTransformer.objectToDbValue(true)!!.toString()
      )
  ...
  private fun assertSimpleSubquery(operator: String, callback: (SelectNode<String, Select1, *>) -> Expr) {
    val expectedBase =
      "SELECT * FROM author WHERE author.name%s(SELECT immutable_value_with_fields.string_value FROM immutable_value_with_fields ) "
    val subQuery = (SELECT
  ...
  private fun assertSameTypeNumericSubquery(operator: String, callback: (SelectNode<Int, Select1, *>) -> Expr) {
    val expectedBase =
      "SELECT * FROM magazine WHERE magazine.nr_of_releases%s(SELECT immutable_value_with_fields.integer FROM immutable_value_with_fields ) "
    val subQuery = (SELECT
  ...
  private fun assertEquivalentTypeNumericSubquery(
    ...
    val expectedBase =
      "SELECT * FROM magazine WHERE magazine.nr_of_releases%s(SELECT immutable_value_with_fields.a_double FROM immutable_value_with_fields ) "
    val subQuery = (SELECT
  ...
  private fun assertEquivalentTypeComplexSubquery(operator: String, callback: (SelectNode<Int, Select1, *>) -> Expr) {
    val expectedBase =
      "SELECT * FROM magazine WHERE magazine.author%s(SELECT immutable_value_with_fields.integer FROM immutable_value_with_fields ) "
    val subQuery = (SELECT
        COLUMN IMMUTABLE_VALUE_WITH_FIELDS.INTEGER
        FROM IMMUTABLE_VALUE_WITH_FIELDS)
  ...
  private fun assertArithmeticExpression(
    ...
    var expected = String.format(
      "SELECT (immutable_value_with_fields.integer%simmutable_value_with_fields.a_short) FROM immutable_value_with_fields ",
      op
    )
    (SELECT
